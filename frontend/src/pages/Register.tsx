...
export const Register: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(error, 'Gagal mendaftar, periksa kembali data Anda.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold text-[var(--text)]">Daftar Akun Baru</h1>
      <p className="mt-3 text-[var(--muted)]">Bergabung dan mulai pelajari SignPath serta tantangan interaktif.</p>
      ...
      <p className="mt-6 text-center text-sm text-[var(--muted)]">
        Sudah punya akun?{' '}
        <Link to={ROUTES.login} className="font-semibold text-[#1e71b7] hover:brightness-110">
          Masuk di sini
        </Link>
      </p>
    </motion.section>
